...
public class Trace
{
    ...
    public static void userfail(String s)
    {
        if(EMIT_USER_ERRORS)
        {
            if(!METHOD_NAMES_ALWAYS)
                ...
                emit(StackParser.getStackMethodBefore("jodychatbot.org.alicebot.server.core.util.", true) + ": " + s, USER_ERROR);
        } else
        {
    ...
    public static void userinfo(String s)
    {
        if(EMIT_USER_INFO)
            if(!METHOD_NAMES_ALWAYS)
                ...
                emit(StackParser.getStackMethodBefore("jodychatbot.org.alicebot.server.core.util.", true) + ": " + s, USER_INFO);
    }
    ...
    public static void userinfo(String as[])
    {
        if(EMIT_USER_INFO)
            if(!METHOD_NAMES_ALWAYS)
                ...
                emit(as, StackParser.getStackMethodBefore("jodychatbot.org.alicebot.server.core.util.", true) + ": ", USER_INFO);
    }

    public static void devfail(String s)
    {
        if(EMIT_DEVELOPER_ERRORS)
            emit(StackParser.getStackMethodBefore("jodychatbot.org.alicebot.server.core.util.", true) + ": " + s, DEVELOPER_ERROR);
        else
    ...
    public static void devinfo(String s)
    {
        if(EMIT_DEVELOPER_INFO)
            emit(StackParser.getStackMethodBefore("jodychatbot.org.alicebot.server.core.util.", true) + ": " + s, DEVELOPER_INFO);
    }
    ...
    private static void emit(Throwable throwable, String s)
    {
        ...
        for(StringTokenizer stringtokenizer = StackParser.getStackTraceFor(throwable); stringtokenizer.hasMoreElements();)
            if(!METHOD_NAMES_ALWAYS)
                ...
                emit(StackParser.getStackMethodBefore("jodychatbot.org.alicebot.server.core.util.", true) + ": " + stringtokenizer.nextToken(), s);
    ...
    private static final String THIS_PACKAGE = "jodychatbot.org.alicebot.server.core.util.";
